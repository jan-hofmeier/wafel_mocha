...
static int handleServer(int connfd) {
    ...
    uint32_t messageBuf;
    int queueId = iosCreateMessageQueue(&messageBuf, 1);
    if (queueId < 0) {
        printf("DK: failed to create message queue\n");
        return -1;
    }

    int timerId = iosCreateTimer(0, 0, queueId, 0xdeafcafe);
    if (timerId < 0) {
    ...
    while (threadRunning) {
        // restart timer
        if (iosRestartTimer(timerId, DK_THREAD_INTERVAL, 0)) {
...
int dkPCHARActivateSmd(IOSVec *vecs) {
    ...
    threadId = iosCreateThread(dkThread, 0, (u32 *) (threadStack + sizeof(threadStack)), sizeof(threadStack), 0x77, 0);
    if (threadId < 0) {
        ret = threadId;
        goto done;
    }

    if (iosStartThread(threadId) < 0) {
